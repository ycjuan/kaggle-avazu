...
fields = ['pub_id','pub_domain','pub_category','device_model','device_conn_type','C14','C17','C20','C21']
...
def convert(src_path, dst_path, is_train):
    with open(dst_path, 'w') as f:
        for i, row in enumerate(csv.DictReader(open(src_path)), start=1):
            ...
            if int(row['sp1_cnt']) < 20:
                feats.append(hashstr('sp1-'+row['sp1_cnt']))
            else:
                feats.append(hashstr('sp1-log-'+str(int(math.log(float(row['sp1_cnt']))))))
