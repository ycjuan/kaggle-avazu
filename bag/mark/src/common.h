...
struct DNode
{
    DNode(uint32_t f, uint32_t j, float v) : f(f), j(j), v(v) {}
    uint32_t f, j;
    float v;
};
...
struct Problem
{
    Problem() : nr_feature(0), nr_instance(0), nr_field(0) {}

    uint32_t nr_feature, nr_instance, nr_field;
    std::vector<DNode> JFV;
    ...
    bool const do_update=false)
{
    ...
    DNode const * const JFV = prob.JFV.data();
    uint64_t const * const P = prob.P.data();
    float * const W = model.W.data();
    ...
    for(uint64_t p1 = P[i]; p1 < P[i+1]; ++p1)
    {
        uint32_t const j1 = JFV[p1].j;
        uint32_t const f1 = JFV[p1].f;
        float const v1 = JFV[p1].v;
        if(j1 >= nr_feature || f1 >= nr_field)
            continue;

        for(uint64_t p2 = p1+1; p2 < P[i+1]; ++p2)
        {
            uint32_t const j2 = JFV[p2].j;
            uint32_t const f2 = JFV[p2].f;
            float const v2 = JFV[p2].v;
            if(j2 >= nr_feature || f2 >= nr_field)
                continue;
            if(f1 == f2)
                continue;
            ...
            __m128 const XMMv = _mm_set1_ps(v1*v2);

            if(do_update)
            {
                __m128 const XMMkappav = _mm_mul_ps(XMMkappa, XMMv);
